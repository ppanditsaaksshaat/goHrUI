    ...
    function weekOffController($scope, $state, $stateParams, $rootScope,
        ...
        function _validateWeekOff(entity) {
            ...
            if (entity.WOSName == undefined || entity.WOSName == "") {
                $rootScope.showMsg("error", "Please enter Set Name");
                return false;
            }
            // alert(entity.WOSGroupId)
            if (entity.WOSGroupId != undefined) {
                if (entity.WOSGroupId.length == 0) {
                    $rootScope.showMsg("error", "Please select atleast one group");
                    return false;
                }
            } else {
                $rootScope.showMsg("error", "Please select atleast one group");
            ...
            if (entity.WOSWEF == undefined || entity.WOSWEF == "") {
                $rootScope.showMsg("error", "Please select With Effect From");
                return false;
            }
            if (entity.WOSWET == undefined || entity.WOSWET == "") {
                $rootScope.showMsg("error", "Please select With Effect To");
                return false;
            }
            ...
            if (gridErrorMsg != "") {
                $rootScope.showMsg("error", gridErrorMsg);
                return false;
            }
            ...
            if (errorMsg != "" && !isWeekOffEdit) {
                $rootScope.showMsg("error", "WeekOff already present from " + errorMsg);
                return false;
            }

            if (parseInt(newWET.diff(newWEF, 'days')) < 30) {
                $rootScope.showMsg("error", "Atleast 1 month difference between with effect from and with effect to");
                return false;
            }
            return true;
