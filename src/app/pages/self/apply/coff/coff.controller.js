...
(function () {
  ...
  function cOffApplyController($scope, $rootScope, $state, $timeout, pageService, editFormService, $filter) {
    ...
    function _attDateChange(event, element, modelCtrl, column) {
      // alert('attchange')
      console.log(event, element, modelCtrl, column)
      ...
      if ($scope.entity.EmpId !== undefined && $scope.entity.EmpId != '' && $scope.entity.EmpId != null) {
        if ($scope.entity.COAttnDate !== undefined && $scope.entity.COAttnDate != '' && $scope.entity.COAttnDate != null) {
          var searchLists = [];
          var searchListData = {
            field: 'EmpId',
            operand: '=',
