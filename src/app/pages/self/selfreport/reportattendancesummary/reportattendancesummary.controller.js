...
(function () {
    ...
    /** @ngInject */
    function reportAttendanceController($scope, $rootScope, $state, $filter, pageService) {
        var vm = this;
        console.log('leave statement')

        $scope.downLoadPdfs = _downLoadPdf;
        ...
        function _getCustomQuerySuccessResult(result) {
            ...
            var leaveStatementData = result[1];
            $scope.imageLogo = result[5][0].FileContent;
            console.log(result[5][0].FileContent)
            var deptName = result[0][0].DeptName;;
            var desgName = result[0][0].DesgName;
            $scope.companyName = result[5][0].CCOName;
            $scope.companyAddress = result[5][0].CCOAddress;


            console.log(leaveStatementData)
            var columnList = [
                { name: 'DATE', displayName: 'Date' },
                { name: 'DayName', displayName: 'Day' },
                { name: 'InTime', displayName: 'In Time' },
                { name: 'OutTime', displayName: 'Out Time' },
                { name: 'DayStatus', displayName: 'Status' }
            ];
            ...
            var docDefinition = {
                ...
                header: {
                    ...
                    columns: [
                        ...
                        {
                            // margin: [250, 35, 0, 0],
                            text: $scope.companyName + '\n' + $scope.companyAddress + '\n' + ' Attendance Summary (Date - ' + moment().format('DD-MMM-YYYY') + ')',
                            style: 'header'
                        },
                        // {
                        //     // margin: [250, 35, 0, 0],
